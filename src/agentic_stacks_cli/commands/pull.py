...
def _validate_repo_url(url: str) -> None:
    """Ensure repo URL is HTTPS from an allowed host."""
    from urllib.parse import urlparse
    parsed = urlparse(url)
    if parsed.scheme not in ("https", "file"):
        raise click.ClickException(f"Refusing to clone non-HTTPS URL: {url}")
    if parsed.scheme == "https" and parsed.hostname not in ALLOWED_HOSTS:
        raise click.ClickException(f"Refusing to clone from untrusted host: {parsed.hostname}")
