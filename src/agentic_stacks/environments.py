...
class StackEnvironmentError(Exception):
    pass
...
def load_environment(env_dir: pathlib.Path) -> dict:
    ...
    if not env_file.exists():
        raise StackEnvironmentError(f"Environment not found: {env_dir}")
    with open(env_file) as f:
        data = yaml.safe_load(f)
    if not isinstance(data, dict):
        raise StackEnvironmentError(f"Environment must be a YAML mapping: {env_file}")
    return data
...
def create_environment(environments_dir: pathlib.Path, name: str, data: dict) -> pathlib.Path:
    ...
    if env_dir.exists():
        raise StackEnvironmentError(f"Environment already exists: {env_dir}")
...
def validate_environment(data: dict, schema: dict) -> None:
    try:
        ...
        raise StackEnvironmentError(str(e)) from e
