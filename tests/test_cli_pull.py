...
def test_pull_by_name(mock_clone, mock_ensure, tmp_path):
    ...
    result = runner.invoke(cli, [
        ...
        "--path", str(tmp_path), "--config", str(config_path),
    ])
...
def test_pull_by_owner_name(mock_clone, mock_ensure, tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, [
        "pull", "agentic-stacks/hardware-dell",
        "--path", str(tmp_path), "--config", str(config_path),
    ])
    assert result.exit_code == 0, result.output
    mock_clone.assert_called_once_with(
        "https://github.com/agentic-stacks/hardware-dell",
        tmp_path / ".stacks" / "hardware-dell",
    )
...
def test_pull_from_lock(mock_clone, tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, [
        "pull", "--path", str(tmp_path), "--config", str(config_path),
    ])
    assert result.exit_code == 0, result.output
